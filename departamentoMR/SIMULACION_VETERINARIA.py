...
class SimulacionVeterinaria:
    def __init__(self,cantidad_dias):
        ...
        self.cantidad_mascotas_por_tabla=[]
    ...
    def crear_tabla_simulacion(self, numeros):
        ...
        for i, numero in enumerate(numeros):
            ...
            mascotas_atendidas += cantidad_mascotas
        ...
        self.cantidad_mascotas_por_tabla.append(mascotas_atendidas)    
    ...
    def resultados_finales(self):
        ...
        axs[0, 0].plot(range(1, len(self.cantidad_mascotas_por_tabla) + 1), self.cantidad_mascotas_por_tabla, marker='o', color='b')
        axs[0, 0].set_title('Cantidad de mascotas atendidas por día')
        axs[0, 0].set_xlabel('Día')
        axs[0, 0].set_ylabel('Cantidad de mascotas')
        axs[0, 0].grid(True)


        labels=['Gastos', 'Ganancias']
        sizes=[total_gastos, total_ganancias]
        colors=['#ff9999', '#66b3ff']
        axs[0, 1].pie(sizes, labels=labels, colors=colors, autopct='%1.1f%%', startangle=140)
        axs[0, 1].set_title('Proporción entre Gastos y Ganancias Totales')
        axs[0, 1].axis('equal')
        axs[0, 1].text(-0.5, 0, f'${total_gastos}', horizontalalignment='center', verticalalignment='center', fontsize=12)
        axs[0, 1].text(-0.5, 0, f'${total_gastos}', horizontalalignment='center', verticalalignment='center', fontsize=12, color='black')
        axs[0, 1].text(0.5, 0, f'${total_ganancias}', horizontalalignment='center', verticalalignment='center', fontsize=12, color='black')

        ventas=[total_alimentos_vendidos, total_medicamentos_vendidos, total_accesorios_vendidos]
        categorias=['Alimentos', 'Medicamentos', 'Accesorios']
        axs[1, 0].bar(categorias, ventas, color=['blue', 'green', 'red'])
        axs[1, 0].set_title('Ventas Realizadas por Categoría')
